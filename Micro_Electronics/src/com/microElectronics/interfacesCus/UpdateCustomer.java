...
public class UpdateCustomer extends JInternalFrame {
	...
	public UpdateCustomer() {
		...
		JLabel lblCustomerForm = new JLabel("Update the Details");
		lblCustomerForm.setBounds(430, 25, 342, 25);
		...
		textFieldCusName.setBounds(152, 117, 133, 20);
		...
		btnAdd.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (val.isEmpty(CTelNo) == false || val.numeric(CTelNo) == false
						|| val.ValidateTelNo(CTelNo) == false) {
					JOptionPane.showMessageDialog(null, "Enter the valid Telephone Number.");
					count++;
				}

				if (count == 0) {
					int select = JOptionPane.showConfirmDialog(null, "Do you want to update this record?",
							"Update confirmation", JOptionPane.YES_NO_OPTION);
					if (select == 0) {
						CustomerM cus = new CustomerM();
						cus.updateCustomer(CID, CName, CTelNo, CAdrs, CDate);
						tableLoad();
						reset();
					}
				}
		...
		btnReset.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				textFieldSearch.setText("Search by Name/TelNo");
				tableLoad();
		...
		textFieldSearch.setBounds(973, 31, 140, 20);
